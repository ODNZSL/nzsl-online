...
module Signbank
  class SignSearchService
    ...
    def initialize(query, relation: Sign)
      @relation = relation
      @query = query
    end
    ...
    def keyword_search(relation)
      relation.joins(
        "INNER JOIN (#{Sign.sanitize_sql_array([SEARCH_SQL, keyword_search_args])}) search_results
        ON search_results.id=#{Sign.table_name}.#{Sign.primary_key}"
      )
    end
    ...
    def handshape_search(relation)
      ...
      handshape_group_ids_clause = handshape_group_ids.map do |hs_id|
        Sign.sanitize_sql_array(['words.handshape LIKE ?', "#{hs_id}.%"])
      end.join(' OR ')

      handshape_ids_clause = (handshape_ids.any? && Sign.sanitize_sql_array(['words.handshape IN (?)', handshape_ids])) || nil
    ...
    def location_search(relation)
      ...
      location_ids += location_groups.flat_map do |lg_id|
        SignMenu.locations.flatten.map do |location_identifier|
      ...
      location_ids_clause = location_ids.map do |loc_id|
        Sign.sanitize_sql_array(['words.location_identifier LIKE ?', "#{loc_id.rjust(2, '0')} - %"])
      end.join(' OR ')
