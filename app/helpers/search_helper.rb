module SearchHelper
    ...
    if number
      ...
      output = content_tag :div, :class => classes_for_sign_attribute(attribute, main) do
        [content_tag(:span, value_for_sign_attribute(number, attribute, main), :class => 'value'),
         image_tag("#{attribute.to_s}s/#{size}/#{attribute.to_s}.#{number.downcase.gsub(/[ \/]/, '_')}.png")].join.html_safe
      end
  ...
  def sign_attribute_image_tag attribute, number
    image_tag("#{attribute.to_s}s/42/#{attribute.to_s}.#{number.downcase.gsub(/[ \/]/, '_')}.png", :class => 'image')
  end
