...
// {{{ public methods 
	...
	extend(self, {
		...
		unload: function(fn) {
          ...
          //* innerHTML caused issues in IE7/8
          //* swapping it out with jquery
					$(wrapper).html(html);
...
(function() {
	...
	function Flash(root, opts, conf) {  
		...
		// express install
			...
			$(root).html(f.getHTML(extend(opts, {src: opts.expressInstall}), {
			...
			if (opts.onFail) {
				...
				if (typeof ret == 'string') { $(root).html(ret); }	
			}			
		}
